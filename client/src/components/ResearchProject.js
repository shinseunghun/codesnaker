        ...
        for(let i=0; i<AlgoList.json.length; i++){
            ...
            result.push(
                <tr className="hidden_type">
                    ...
                    <td><Link to={'/ResearchProjectView/'+data.algo_code}>{data.algo_function}</Link></td>
                    <td>{reg_date}</td>
                    <td>{data.algo_name}</td>
                    <td>
                        <Link to={'/ResearchProjectView/'+data.algo_code} 
                        className="bt_c1 bt_c2 w50_b">수정</Link>
                         <a href="#n" class="bt_c1 w50_b" id={data.algo_code}
                        onClick={(e) => this.deleteAlgo(e)}>삭제</a>
                    </td>
                </tr>
            )
        }
            ...
            <section className="sub_wrap" >
                <article className="s_cnt mp_pro_li ct1 mp_pro_li_admin">
                    <div className="li_top">
                        <h2 className="s_tit1">알고리즘 문제</h2>
                        <div className="li_top_sch af">
                        <Link to={'/ResearchProjectView/register'} className="sch_bt2 wi_au">문제 등록</Link>
                        </div>
                    </div>
